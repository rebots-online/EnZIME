...
function renderArchiveHTML(result){
 ...
 frame.addEventListener('load',()=>{const d=frame.contentDocument;if(!d)return;d.addEventListener('click',event=>{const a=event.target.closest('a');if(!a)return;if(a.hasAttribute('data-external')){event.preventDefault();status(`External reference: ${a.getAttribute('data-external')} (copy the address to open outside the offline reader).`);return;}const href=a.getAttribute('href');if(!href||href.startsWith('#'))return;event.preventDefault();openArchiveLink(href,result,frame).catch(report);});d.addEventListener('selectionchange',()=>{if(frame.isConnected&&state.source?.id===result.source.id)captureSelection(d.getSelection()?.toString());});});
 $('reading').replaceChildren(frame);
}
async function openArchiveLink(href,result,frame){
 const target=new URL(href,location.origin),base=result.resourceBase||'/api/resources/';
 if(target.origin!==location.origin||!target.pathname.startsWith(base))throw Error('Invalid archive resource link.');
 const ticket=state.opening,response=await fetch(target.pathname,{method:'HEAD'});
 if(!response.ok)throw Error(`Resource unavailable (${response.status}).`);
 if(ticket!==state.opening||!frame.isConnected||state.source?.id!==result.source.id)return;
 const mime=(response.headers.get('Content-Type')||'').split(';')[0].trim().toLowerCase();
 if(mime.startsWith('text/')||mime==='application/xhtml+xml'){await navigateKey(decodeURIComponent(target.pathname.slice(base.length)));return;}
 const link=el('a',{href:target.href,target:'_blank',rel:'noopener noreferrer'},'Open linked resource');
 status('');$('status').append(link);window.open(target.href,'_blank','noopener,noreferrer');
}
...
async function drawPdf(){
 ...
 if(state.source?.locator?.page!==state.page){clearSelection();state.source=null;}
...
function clearSelection(){state.selection='';$('note-anchor').textContent='Select text in the reader to anchor your note.';}
function captureSelection(text){if(!state.source||!text?.trim())return;state.selection=text.trim().slice(0,4000);$('note-anchor').textContent=`Quote: “${state.selection.slice(0,150)}${state.selection.length>150?'…':''}”`;}
...
async function openSource(id,citation){
 ...
 state.current=item;clearSelection();show('reader');$('pdf-tools').hidden=true;$('entry-search-form').hidden=source.kind!=='zim';$('entry-list').replaceChildren();$('more-entries').hidden=true;
...
async function sendChat(){
 ...
 state.chatId??=crypto.randomUUID();chatAbort=new AbortController();$('stop-chat').hidden=false;$('send-chat').disabled=true;let text='',citations=[];
 try{
  const res=await fetch('/api/chat',{...post({chatId:state.chatId,message,sourceId:state.source?.id,model:$('model-select').value||state.settings.model,messages:state.messages.map(({role,content})=>({role,content}))}),signal:chatAbort.signal});if(!res.ok)throw Error((await res.json()).error);
...
function setDraft(draft=null,correction=null){state.draft=draft;state.correction=correction;state.draftVersion++;}
async function loadDrafts(){const drafts=await api('/api/drafts');$('draft-list').replaceChildren(...drafts.map(d=>el('button',{class:'draft-item',onclick:()=>{setDraft(d);$('draft-title').value=d.title;$('draft-body').value=d.body;$('draft-status').textContent='Private draft';}},d.title)));}
async function saveDraft(finalize=false){
 state.draft??={id:crypto.randomUUID(),sourceRefs:state.source?[state.source]:[]};
 const version=state.draftVersion,snapshot={id:state.draft.id,title:$('draft-title').value,body:$('draft-body').value,format:'markdown',sourceRefs:state.draft.sourceRefs??[]};
 const operation=draftSaveQueue.then(async()=>{const draft=await api('/api/drafts',post(snapshot));if(finalize)await api('/api/finalize',post({id:draft.id}));return draft;});
 draftSaveQueue=operation.catch(()=>{});const saved=await operation;
 if(version===state.draftVersion&&state.draft?.id===snapshot.id){state.draft=saved;$('draft-status').textContent=$('draft-title').value===snapshot.title&&$('draft-body').value===snapshot.body?(finalize?'Finalized':'Draft saved'):'Unsaved changes';}
 await loadDrafts();return saved;
}
...
$('reader-correct').onclick=run(async()=>{if(!state.source)throw Error('Open a source passage first.');const source=state.source,ticket=state.opening,version=state.draftVersion,r=await api(`/api/assets/${state.current.id}/read?key=${encodeURIComponent(state.key)}&page=${state.page}`);if(ticket!==state.opening||source!==state.source||version!==state.draftVersion)return;setDraft(null,source);$('draft-title').value=source.title+' · correction';$('draft-body').value=r.text;$('draft-status').textContent='Correction overlay · original preserved';show('create');});
...
$('new-draft').onclick=()=>{setDraft();$('draft-title').value='';$('draft-body').value='';$('draft-status').textContent='New private draft';};$('draft-form').onsubmit=run(e=>{e.preventDefault();return saveDraft();});
$('finalize-draft').onclick=run(async()=>{const title=$('draft-title').value,body=$('draft-body').value,version=state.draftVersion,correction=state.correction;if(!title.trim()||!body.trim())throw Error('Give your work a title and text.');if(correction){await api('/api/overlay',post({sourceId:correction.id,text:body}));if(version===state.draftVersion){if(title===$('draft-title').value&&body===$('draft-body').value){state.correction=null;$('draft-status').textContent='Finalized';}else $('draft-status').textContent='Unsaved changes';}status('Correction saved as an immutable overlay. Active retrieval uses the correction.');}else{await saveDraft(true);status('Final revision saved. Earlier revisions remain available.');}await refresh();});
...
$('save-answer').onclick=run(async()=>{const last=[...state.messages].reverse().find(m=>m.role==='assistant');if(!last)throw Error('Generate an answer first.');setDraft();$('draft-title').value='From a conversation with my library';$('draft-body').value=last.content;show('create');await saveDraft();});
