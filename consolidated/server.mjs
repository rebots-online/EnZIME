...
export async function createApp({root,checkout=null}={}){
 ...
 if(state){if(req.method==='GET')return json(200,store.readState(state[1]));if(req.method==='POST'){const chunks=[];let size=0;for await(const c of req){size+=c.length;if(size<=65536)chunks.push(c);}if(size>65536)return json(413,{error:'State too large'});const value=JSON.parse(Buffer.concat(chunks).toString('utf8'));store.saveState(state[1],value);return json(200,value);}}
 ...
 if(range){const m=range.match(/^bytes=(\d*)-(\d*)$/);res.setHeader('Content-Range',`bytes */${item.size}`);if(!m||(!m[1]&&!m[2])||!item.size)return json(416,{error:'Unsupported range'});if(!m[1]){const length=Number(m[2]);if(!Number.isSafeInteger(length)||length<=0)return json(416,{error:'Invalid range'});start=Math.max(0,item.size-length);}else{start=Number(m[1]);const requestedEnd=m[2]?Number(m[2]):end;if(!Number.isSafeInteger(requestedEnd))return json(416,{error:'Invalid range'});end=Math.min(requestedEnd,end);}if(!Number.isSafeInteger(start)||start>end||start>=item.size)return json(416,{error:'Invalid range'});res.setHeader('Content-Range',`bytes ${start}-${end}/${item.size}`);}
